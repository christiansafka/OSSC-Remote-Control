...
static int keyboard_command_brakes();
static int keyboard_command_throttle();
static int keyboard_command_steering();
...
int check_for_controller_update( )
{
    ...
    if ( control_enabled )
    {
		if ( keyboard_enabled )
		{
			return_code = keyboard_command_brakes();

        	if ( return_code == OSCC_OK )
        	{
        	    return_code = keyboard_command_throttle();
        	}

        	if ( return_code == OSCC_OK )
        	{
        	    return_code = keyboard_command_steering();
        	}
		}
        else
		{
...
static int keyboard_command_brakes()
{
    ...
    if ( commander_enabled == COMMANDER_ENABLED && control_enabled == true )
    {
        double normalized_position = brake_value;

        return_code = OSCC_OK; //get_normalized_position( JOYSTICK_AXIS_BRAKE, &normalized_position );
...
static int keyboard_command_throttle()
{
    ...
    if ( commander_enabled == COMMANDER_ENABLED && control_enabled == true )
    {
        double normalized_throttle_position = throttle_value;

        return_code = OSCC_OK; //get_normalized_position( JOYSTICK_AXIS_THROTTLE, &normalized_throttle_position );

        if ( return_code == OSCC_OK && normalized_throttle_position >= 0.0 )
        {
            double normalized_brake_position = brake_value;

            // If braking, do not throttle
            return_code = OSCC_OK; //get_normalized_position( JOYSTICK_AXIS_BRAKE, &normalized_brake_position );
...
static int keyboard_command_steering()
{
    ...
    if ( commander_enabled == COMMANDER_ENABLED && control_enabled == true )
    {
        double normalized_position = steering_value;

        return_code = OSCC_OK; //get_normalized_position( JOYSTICK_AXIS_STEER, &normalized_position );
